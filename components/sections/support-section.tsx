...
export function SupportSection() {
  ...
  return (
    <section 
      id="support" 
      ref={sectionRef} 
      className="relative overflow-hidden bg-card py-20 md:py-28" // 기존 bg-card 유지
    >
      {/* 1. 페럴렉스 배경 이미지 (최하단 레이어 z-0) */}
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        <img
          ...
          style={{ 
            ...
            opacity: 0.25 // 기존 텍스트 가독성을 위해 이미지 투명도를 조절 (검은 화면 방지)
          }}
        />
      </div>

      {/* 2. 기존의 패럴랙스 원형 데코레이션 배경 (z-10) */}
      <div className="pointer-events-none absolute bottom-0 right-0 h-[500px] w-[500px] translate-x-1/4 translate-y-1/4 rounded-full bg-primary/3 z-10" />
      <div className="pointer-events-none absolute left-0 top-0 h-[300px] w-[300px] -translate-x-1/4 -translate-y-1/4 rounded-full bg-accent/3 z-10" />
      
      {/* 3. 콘텐츠 영역 (최상단 레이어 z-20) */}
      <div className="relative z-20 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mx-auto max-w-4xl">
          {/* 후원금 사용 안내 박스 (기존의 밝은 그라데이션 스타일 그대로 복구) */}
          <div
            className={`mb-10 rounded-lg bg-gradient-to-r from-slate-50/90 to-blue-50/90 backdrop-blur-sm p-8 shadow-md transition-all duration-700 delay-200 ${
              isVisible ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
            }`}
          >
